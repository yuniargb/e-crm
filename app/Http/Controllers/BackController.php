...
class BackController extends Controller
{
    ...
    public function tambahPromosi()
    {
        ...
        $paket = DB::select("select p.id,p.nama_paket,p.harga from pakets p
        left join (select * from promos where tgl_selesai >= '".date('Y-m-d')."') ps on ps.paket_id=p.id
        WHERE ps.id  is null 
        GROUP BY p.id,p.nama_paket,p.harga");
